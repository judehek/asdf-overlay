...
fn should_filter_message(msg: &MSG) -> bool {
    ...
    for backend in Backends::iter() {
        ...
        // No `GetCapture() == backend.id` shortcut here: the backend id IS
        // the game's own top-level HWND, so the game using `SetCapture` on
        // its own window for unrelated reasons (Valorant does this for
        // normal UI input) would make that predicate true and we'd drop
        // every cursor message until the game released its own capture --
        // wedging the game completely. Hit-testing on cursor position
        // alone is the correct rule.
